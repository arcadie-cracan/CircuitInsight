...
def pole_scale_of(system: MnaSystem) -> float:
    """Geometric-mean magnitude of the uncompensated finite natural
    frequencies -- ONE symbolic s-determinant. Candidate sweeps reuse it
    across every multi_locus call on the same system."""
    droots = _scaled_roots(sp.Poly(sp.expand(_det(_numeric_A(system))), S))
    mag = np.abs(droots[np.abs(droots) > 0])
    return float(np.exp(np.mean(np.log(mag)))) if mag.size else 1.0


def multi_locus(system: MnaSystem, pairs, pole_scale: float | None = None):
    ...
    # the circle sits at the pole scale, so coefficient recovery is well
    # conditioned. Correctness does not depend on rho (any M > deg samples
    # fix the polynomial); only conditioning does. The symbolic determinant
    # behind it depends ONLY on `system`, so callers sweeping many candidate
    # branch sets over one system (the suggester) compute pole_scale() once
    # and pass it in -- Iordache's latency principle applied inside a run.
    rho = pole_scale if pole_scale is not None else pole_scale_of(system)
...
def suggest_multi_compensation(system: MnaSystem, probe: str, *,
                               ...
                               candidates=None, kappa: float = 0.05,
                               c_grid=None, r_grid=None, mirror=None,
                               min_gain_improve: float = 0.02,
                               torn=None,
                               cache: dict | None = None) -> MultiSuggestion:
    ...
    # `torn` = (primitives, ground, cut) matching `system`: the run's
    # symbolic work (the T0 loop gain, the pole scale, the candidate
    # screen) is then served through / keyed on the tearing latency cache,
    # so a RE-RUN -- the GUI's edit-and-resuggest cycle, a target sweep,
    # a goal change -- pays near zero symbolic determinants for the
    # unchanged circuit, and after an edit only the changed half of the
    # loop gain re-solves (docs/interp-speedup-plan.md S-E).
    fp = None
    if torn is not None:
        from .tearing import _fingerprint, split_loop_gain
        t_prims, t_ground, t_cut = torn
        fp = _fingerprint(t_prims, t_ground)
        if t_cut is not None:
            T0 = split_loop_gain(t_prims, t_ground, probe, t_cut, keep=[],
                                 cache=cache)
        elif cache is not None:
            # no usable cut (e.g. mirrored fd halves): still reuse the
            # whole T0 across runs by content fingerprint
            key = ("suggest_T0", fp, probe)
            T0 = cache.get(key)
            if T0 is None:
                T0 = loop_gain(system, probe, [])
                cache[key] = T0
    ...
    if cache is not None and fp is not None:
        key = ("suggest_rho", fp)
        rho_run = cache.get(key)
        if rho_run is None:
            rho_run = pole_scale_of(system)
            cache[key] = rho_run
    else:
        rho_run = pole_scale_of(system)  # ONE symbolic det per run
