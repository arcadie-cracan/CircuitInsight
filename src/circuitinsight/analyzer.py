...
class Analyzer:
    ...
    def template_form(self, inp: str, out: str, keep=(), *,
                      budget: float = 0.05, **kw):
        """Render tf(inp, out, keep) in the standard multistage form: DC
        gain, GBW, and a short symbolic formula per pole/zero, each
        checked against the exact numeric root and reported with its
        displacement. See analysis/template.py."""
        from .analysis.template import template_form as _tpl

        return _tpl(self.tf(inp, out, keep, **kw), budget=budget)

    def lump_report(self, *, nodes=None, kinds=("c", "g", "r")):
        """Which parallel ground-referred passives could be folded into one
        symbol each (C_node and friends), and how many symbols that saves.
        Exact -- parallel same-kind elements add. Pairs with
        scan_ac_grounds: grounding is what MAKES elements ground-referred.
        See analysis/lumping.py."""
        from .analysis.lumping import lump_report as _rep

        return _rep(self.primitives, self.flat.ground, nodes=nodes,
                    kinds=kinds)

    def scan_ac_grounds(self, inp: str, out: str, *,
                        budget_db: float = 0.1, **kw):
        """Which nodes are really AC grounds for tf(inp -> out), and what
        does assuming so cost? Nominates the mirror/bias nodes
        structurally and scores EVERY candidate exactly (Sherman-Morrison
        on one inverse per frequency), then recommends the largest set
        within the budget. See analysis/acground.py."""
        from .analysis.acground import scan_ac_grounds as _scan

        return _scan(self.primitives, self.flat.ground, inp, out,
                     budget_db=budget_db, alias=self._alias, **kw)

    def advise_split(self, inp: str, out: str, keep=(), **kw):
        """Should this circuit be TORN for the solve, and where? Ranks the
        vertex cuts by whether tearing actually pays (balance of the two
        halves and how the keep-set splits), and when none pays, proposes
        AC-ground designations that would fix the balance -- each with the
        dB/degree error it introduces, measured. See analysis/tearing.py."""
        from .analysis.tearing import advise_split as _advise

        return _advise(self.primitives, self.flat.ground, inp, out,
                       keep=keep, alias=self._alias, **kw)
