...
class TransferFunction:
    ...
    def numeric(self, f):
        ...
        return numeric_eval(self.numeric_expr(), self.s, f)
...
def numeric_eval(expr: sp.Expr, s: sp.Symbol, f) -> np.ndarray:
    """Evaluate a frequency-domain expression at f (Hz) -> complex ndarray.

    Fast numpy evaluation; for large circuits whose exact integer
    coefficients overflow float64 (a 40-device amplifier's determinant has
    coefficients with hundreds of digits), falls back to
    arbitrary-precision mpmath per point -- the RATIO is O(gain) and fits
    even when the numerator and denominator separately do not. Shared, so
    every consumer inherits the fallback instead of rediscovering the
    overflow."""
    ...
    # a degenerate expression (zoo/nan from e.g. an over-aggressive prune
    # candidate whose denominator collapsed) is a BAD VALUE, not a crash:
    # NaNs make every caller's error metric infinite, so the candidate is
    # rejected and the caller backs off. Lambdify would instead die with
    # KeyError('ComplexInfinity') deep in its printer.
    if expr.has(sp.zoo, sp.nan, sp.oo, -sp.oo):
        return np.full(w.shape, complex("nan"))
    ...
    try:
        ...
        fn = sp.lambdify(s, expr, "mpmath")
        with mpmath.workdps(50):
            out = _shape([complex(fn(mpmath.mpc(0, wi.imag))) for wi in w])
    return out
...
def solve_tf_batch(
    ...
    """Multi-output transfer functions over ONE shared matrix.

    `tasks` is a list of (system, output) pairs whose systems must all
    share the same matrix A — only the excitation vector z (chosen by
    build_mna's input_name) and the output unknown differ. This is the
    Tan-Shi hierarchical-suppression shape (several cofactors over one
    determinant): the denominator work is paid once for the whole batch,
    so m outputs cost roughly (1+m)/(2m) of m separate solves. The
    tearing sub-solves are the intended caller.

    keep/method follow solve_tf; outputs may be node names or raw
    unknown indices (branch currents)."""
    if not tasks:
        return []
    sys0 = tasks[0][0]
    for system, _ in tasks[1:]:
        if system.A != sys0.A:
            raise MnaError(
                "solve_tf_batch: all systems must share the same matrix "
                "(same primitives; only input_name may differ)")
    ...
    if method != "direct" and not is_all(keep):
        _, kept_names = hybrid_split(sys0, keep)
        if not kept_names:
            # ALL-NUMERIC: sample s and reconstruct, rather than carry
            # polynomial arithmetic through a symbolic QQ[s] elimination.
            # Exact (the s-degree bound is structural), an order of
            # magnitude faster on real circuits, and -- unlike sympy's
            # det() -- it reports progress and can be cancelled.
            from .interp import solve_tf_interp_batch

            return solve_tf_interp_batch(tasks, keep, progress=progress)
        grid = 1
        for n in kept_names:
            grid *= max(1, sys0.stamp_counts.get(n, 1)) + 1
        if method == "interp" or grid > 4:
            from .interp import solve_tf_interp_batch

            return solve_tf_interp_batch(tasks, keep, progress=progress)

    # direct path: substitutions and the denominator paid once
    A = sys0.A
    if not is_all(keep):
        subs, _ = hybrid_split(sys0, keep)
        A = A.xreplace(subs)
    den = _det(A)
    if den == 0:
    ...
    for system, output in tasks:
        ...
        elif output in system.node_index:
        ...
        out.append(TransferFunction(
            expr=sp.cancel(num / den), values=dict(system.values),
...
def solve_tf(
    ...
    if method != "direct" and not is_all(keep):
        ...
        if not kept_names:
            # ALL-NUMERIC: sample s and reconstruct instead of carrying
            # polynomial arithmetic through a symbolic QQ[s] elimination.
            # Exact, an order of magnitude faster on real circuits, and it
            # reports progress and can be cancelled -- sympy's det() can
            # do neither.
            from .interp import solve_tf_interp

            return solve_tf_interp(system, output, keep, progress=progress)
