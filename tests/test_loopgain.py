...
def test_loop_gain_forwards_progress_through_the_session(monkeypatch):
    """The loop-gain bench was the last multi-minute blind spot: the GUI
    launched it with no callback, so a hybrid solve ran behind a busy
    bar with a dead Cancel. The session must FORWARD the callback to
    the analyzer (whether the solver then reports depends on its route:
    a small keep may take the direct determinant, which is the one
    documented unreportable path -- the engine's own reporting is
    covered by test_solve_reports_progress)."""
    ...
    with warnings.catch_warnings():
        ...
        c = SessionController.open(FIXTURES / "tb_ota2s_stb.cin.json",
                                   FIXTURES / "psf_stb")
        seen = {}
        orig = Analyzer.loop_gain

        def spy(self, probe, keep=(), progress=None):
            seen["progress"] = progress
            return orig(self, probe, keep=keep, progress=progress)

        monkeypatch.setattr(Analyzer, "loop_gain", spy)
        cb = lambda d, t: None                       # noqa: E731
        c.loop_gain("IPRB0", keep=["I0.Cc"], progress=cb)
