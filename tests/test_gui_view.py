...
def test_eng_coeff_tex():
    assert view._eng_coeff_tex(5.936e-5) == r"59.36\,\mathrm{µ}"
...
def test_latex_eng_no_scientific_notation():
    ...
    assert r"\mathrm{µ}" in s and r"\mathrm{f}" in s             # micro and femto present
...
def test_latex_eng_coefficient_first_with_cdot():
    ...
    assert out2 == r"25.76\,\mathrm{µ} \cdot g_{m,\mathrm{MP0}}"
...
def test_mirror_map_pairs_p_and_n_nodes():
    """Trailing and MEDIAL p/n both pair (the fd bench's Miller port is
    outpi/outni); a single-ended design yields no pairs."""
    m = view.mirror_map(["outp", "outn", "outpi", "outni", "n1p", "n1n",
                         "I0.netp", "I0.netn", "vcm_sense", "tail"])
    assert m == {"outp": "outn", "outpi": "outni", "n1p": "n1n",
                 "I0.netp": "I0.netn"}
    assert view.mirror_map(["I0.net1", "vout", "vdd!"]) == {}


def test_mirror_map_drops_ambiguous_names():
    """A name that could mirror onto two partners is dropped, not guessed."""
    assert "pp" not in view.mirror_map(["pp", "pn", "np"])


def test_whatif_survives_overflowing_coefficients():
    """The what-if sliders keep a handful of symbols and freeze the rest as
    the EXACT rationals of the operating point. On a large circuit those
    rationals have hundreds of digits, so the compiled numpy function
    raises "int too large to convert to float" -- the evaluator must fall
    back to extended precision, as the plain TF evaluator already does."""
    import numpy as np
    import sympy as sp

    from circuitinsight.engine.mna import TransferFunction

    s, gm = sp.Symbol("s"), sp.Symbol("gm", positive=True)
    big = sp.Integer(10) ** 320
    tf = TransferFunction(expr=big * gm / (big * sp.Integer(10) ** 12
                                           * (1 + s / sp.Integer(10) ** 3)),
                          values={"gm": 1e-3}, symbols={"gm": gm})

    class _R:                                # the two attributes it reads
        keep, tf_ = ["gm"], tf

    r = _R()
    r.tf = tf
    names, evaluate = view.whatif_fn(r)
    assert names == ["gm"]
